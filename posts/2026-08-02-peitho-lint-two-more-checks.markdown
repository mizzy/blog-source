...
CSSを解析する方向は最初から捨てた。原理的にはできなくはないんだろうけど、カスケードや継承を自前で解決して、`em`や`%`や`clamp()`を実効値まで畳んで……とやっていくと、結局ブラウザのやっていることを再実装することになる。しかもlintはもともとヘッドレスChromeを起動しているので、`getComputedStyle`を呼べば答えが返ってくる。
