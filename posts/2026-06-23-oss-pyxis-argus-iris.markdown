...
名前は羅針盤座（Pyxis）から。[Carina](https://github.com/carina-rs/carina)が船骨座で、どちらも、現在は4つの星座に分割されたアルゴ座（Argo Navis）の一部にあたる星座。CarinaのplanをAIが解説する、つまり羅針盤が船に方向を示す、という対比でつけた。
