...
### 「24pt」と表示しながら24pt未満と警告する問題

判定を計算済みpxで、表示をptで、それぞれ別々に丸めていたら、レポートが「24ptのテキストが24pt未満です」と言う窓ができてしまった。今は表示するpt値（0.1pt単位に丸めたもの）で判定するようにしてある。表示と判定が同じ値を見ていれば、この自己矛盾は起きない。

---

## スロット単位のはみ出し検査

もうひとつはこっちが本題で、既存のはみ出し検査に穴が空いていた。
