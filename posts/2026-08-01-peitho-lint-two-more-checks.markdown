...
24ptを下回るテキストがあるスライドに警告を出すようにした。
...
既存のはみ出し検査が「内容が多すぎて枠から出た」を捕まえるのに対して、こっちは「枠に収めるために縮めた」を捕まえる。後者は見た目が壊れないので、ビルドも通るし目視でも気づきにくい。会場の後ろから読めないことに気づくのは本番中、というやつ。

閾値の24ptは固定で、frontmatterで変えられるようにはしていない。Peithoのスライドは高さが常に720pxで（16:9なら1280x720、4:3なら960x720）、CSSの24ptは32pxに相当する。720分の32という比率は、PowerPointの7.5インチのスライドで24ptと言っているのと同じ比率なので、アスペクト比が変わっても意味のある値になる。
