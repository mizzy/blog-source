...
除外したいものは「KaTeXのMathML」ではなく「視覚的に隠れている要素」なので、その性質を直接見る形にした。具体的には、1x1px以下に畳まれているか、`visibility`で消されているか、を見ている。

なお最初はここに「`clip`や`clip-path`で切り抜かれているか」も入れていたんだけど、これは外した。角丸を付けただけの要素まで「隠れている」と判定してしまって、本物のはみ出しを見逃すようになったため。
